...
@Path("/rpc-enable-user")
@Produces(MediaType.APPLICATION_JSON)
public class RpcEnableUser {


    private static final Logger LOGGER = LoggerFactory.getLogger(RpcEnableUser.class);
    private final Validator validator;
    @Context
    ServletContext context;

    public RpcEnableUser(Validator validator) {
    ...
    public Response rpcEnableUser(
        ...
        LOGGER.info("POST rpcEnableUser [" + userEmail + "] Remote Host[" +
                req.getRemoteHost() +
                "] Remote Address[" + req.getRemoteAddr() +
                "] Remote Port[" + String.valueOf(req.getRemotePort()) + "].");
        try {

            adminEnableUser(userEmail);
            return Response
            ...
            throw new CustomException(
                    ...
                    "ERR-500-RPC-ENABLE-USER",
                    "process-exception-rpc-enable-user",
                    ex);
        }
    }


    protected static AdminEnableUserResponse adminEnableUser(String userEmail) {

        AdminEnableUserResponse AdminEnableUserResponse = null;
        ...
        try ( CognitoIdentityProviderClient cognitoclient = CognitoIdentityProviderClient.builder()
                ...
                .build()){
            AdminEnableUserResponse = cognitoclient.adminEnableUser(
