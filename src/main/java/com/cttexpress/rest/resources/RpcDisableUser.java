...
@Path("/rpc-disable-user")
@Produces(MediaType.APPLICATION_JSON)
public class RpcDisableUser {


    private static final Logger LOGGER = LoggerFactory.getLogger(RpcDisableUser.class);
    private final Validator validator;
    @Context
    ServletContext context;

    public RpcDisableUser(Validator validator) {
    ...
    @RolesAllowed({"ADMIN"})
    @Path("/{userEmail}")
    public Response rpcDisableUser(
        ...
        LOGGER.info("POST rpcDisableUser [" + userEmail + "] Remote Host[" +
                req.getRemoteHost() +
                "] Remote Address[" + req.getRemoteAddr() +
                "] Remote Port[" + String.valueOf(req.getRemotePort()) + "].");
        try {

            adminDisableUser(userEmail);
            return Response
            ...
            throw new CustomException(
                    ...
                    "ERR-500-RPC-DISABLE-USER",
                    "process-exception-rpc-disable-user",
                    ex);
        }
    }
    ...
    protected static AdminDisableUserResponse adminDisableUser(String userEmail) {
        ...
        try ( CognitoIdentityProviderClient cognitoclient = CognitoIdentityProviderClient.builder()
                ...
                .build()){
            AdminDisableUserResponse = cognitoclient.adminDisableUser(
